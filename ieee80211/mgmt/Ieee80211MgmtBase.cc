...
void Ieee80211MgmtBase::initialize(int stage)
{
    if (stage==0)
    {
        ...
        if (par("UseQos80211e").boolValue()){
        	EV << " QOS Enabled " << endl;
        	useQos = true;
        }
        else {
        	EV << " QOS Disabled " << endl;
        	useQos = false;
        }

    	std::cout << " creo classifier object " << std::endl;

        if (hasPar("classifierClass"))
        {
        	EV << " QOS using classifier: " << par("classifierClass").stringValue() <<endl;
...
{
	std::cout << " MGMNT ENQUEUEEEEEEEEEEEE " << std::endl;

	std::cout << " MGMNT: " << useQos << std::endl;
    ...
    {
    	//conta quanti ce ne sono per tipo di traffico
